...
std::unique_ptr<program> generate_asm(std::unique_ptr<program> prog);
...
int main() {
	...
	prog = generate_asm(std::move(prog));
	...
	std::system("\"C:\\Program Files\\LLVM\\bin\\clang\" -v test.o -o test.exe -Wl,/ENTRY:_main,/SUBSYSTEM:CONSOLE");
...
std::unique_ptr<program> generate_asm(std::unique_ptr<program> prog) {
	...
	if (!file.is_open()) {
		std::cout << "Couldn't open test.asm.\n";
		return nullptr;
	}

	file << "\t.section .text\n";
	file << "\t.global _" << prog->functions[0]->name << "\n";
	file << "_" << prog->functions[0]->name << ":\n";
	file << "\tmov $2, %eax\n";
	file << "\tret\n";
